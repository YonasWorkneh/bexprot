...
import { Loader2, CheckCircle, XCircle, ArrowDownCircle, Copy, FileText, ExternalLink } from "lucide-react";
...
const AdminDepositsPanel = () => {
    ...
    const fetchDeposits = async () => {
        console.log('[Admin Deposits] Fetching deposits...');
        try {
            // Fetch deposits with user data from crypto_deposits
            ...
            console.log('[Admin Deposits] Query SUCCESS - found', data?.length || 0, 'deposits');
            if (data) {
                const depositsWithUsers = data.map((d: any) => ({
                    ...d,
                    user: d.user
                }));
                console.log('[Admin Deposits] Setting deposits state:', depositsWithUsers);
                setDeposits(depositsWithUsers);
            } else {
                console.log('[Admin Deposits] No deposits found');
                setDeposits([]);
            }
        } catch (error: any) {
            console.error('[Admin Deposits] Fetch deposits error:', error);
            toast.error("Failed to fetch deposits: " + error.message);
            setDeposits([]);
        } finally {
            setIsLoading(false);
            console.log('[Admin Deposits] Fetch completed');
        }
    };

    const handleStatusUpdate = async (deposit: CryptoDeposit, action: 'approve' | 'reject') => {
        try {
            ...
            if (action === 'approve') {
                // Credit the user
                const result = await creditUserDeposit(deposit.id, user.id, 1, "Admin approved via panel");
                if (result.success) {
                    toast.success(`✅ Deposit approved! $${deposit.amount_usd} credited to user.`);
                } else {
                    throw new Error(result.error);
                }
            } else if (action === 'reject') {
                const result = await rejectDeposit(deposit.id, user.id, "Rejected by admin via panel");
                if (result.success) {
                    ...
                    <div className="flex items-center justify-between">
                        <CardTitle className="flex items-center gap-2">
                            ...
                            <Badge variant="secondary" className="ml-2">
                                {deposits.filter(d => d.status === 'pending' || d.status === 'reported').length} Pending
                            </Badge>
                        </CardTitle>
                        <Button size="sm" variant="outline" onClick={() => fetchDeposits()}>
                            Refresh
                        </Button>
                    </div>
                </CardHeader>
                            ...
                            {deposits.map((deposit) => (
                                <TableRow key={deposit.id} className="hover:bg-muted/30 transition-colors">
                                    <TableCell className="pl-6 font-medium">
                                        <div>
                                            <div className="font-semibold">{deposit.user?.name || 'Unknown User'}</div>
                                            <div className="text-xs text-muted-foreground">{deposit.user?.email || 'No Email'}</div>
                                            <div className="text-[10px] text-muted-foreground mt-1 font-mono">{deposit.deposit_code}</div>
                                        </div>
                                    </TableCell>
                                    <TableCell>
                                        <div className="font-mono font-bold">${deposit.amount_usd?.toFixed(2) || deposit.amount?.toFixed(2)}</div>
                                        ...
                                        {deposit.transaction_hash ? (
                                            <div className="flex items-center gap-2">
                                                ...
                                                <Button
                                                    ...
                                                    className="h-6 w-6"
                                                    onClick={() => {
                                                        navigator.clipboard.writeText(deposit.transaction_hash!);
                                    ...
                                    <TableCell>
                                        <div className="text-sm text-muted-foreground">
                                            ...
                                            <div className="text-xs opacity-70">{new Date(deposit.created_at).toLocaleTimeString()}</div>
                                        </div>
                                    </TableCell>
                                    <TableCell>
                                        <Badge className={`capitalize ${
                                            deposit.status === 'credited' || deposit.status === 'confirmed' ? 'bg-green-500/15 text-green-600 border-green-200' :
                                            deposit.status === 'pending' || deposit.status === 'reported' ? 'bg-yellow-500/15 text-yellow-600 border-yellow-200' :
                                    ...
                                    <TableCell className="text-right pr-6">
                                        {(deposit.status === 'pending' || deposit.status === 'reported') && (
